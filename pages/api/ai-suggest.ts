...
- "Excited and energetic? That's fantastic! What's got you so pumped up today?"`;
  ...
  if (cuisine && cuisine !== 'Other') {
    cuisineLine = `\nThe user prefers ${cuisine} cuisine. Focus on suggesting ${cuisine} dishes or dishes that are commonly found in ${cuisine} restaurants.`;
  }
...
IMPORTANT: Always respect the user's cuisine preference. If they prefer a specific cuisine, suggest dishes from that cuisine or similar styles.
${cuisineLine}
${spicyLine}
...
export default async function handler(
  ...
  try {
    const systemPrompt = buildSystemPrompt(spicy, cuisine || '', mode);
